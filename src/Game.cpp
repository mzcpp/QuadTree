	...
	circle_texture_(nullptr)
{
	...
	const Rect<NumType> area = { static_cast<NumType>(0), static_cast<NumType>(0), static_cast<NumType>(w), static_cast<NumType>(h) };
	...
	circle_texture_ = std::make_unique<CircleTexture<NumType>>(renderer_, center, circle_r_, color);
}
...
void Game::HandleEvents()
{
	...
	while (SDL_PollEvent(&e) != 0)
	{
		...
		if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT)
		{
			...
			NumType ph = static_cast<NumType>(30);

			QtItemType p = { px - (pw / 2), py - (ph / 2), pw, ph };
			Rect<NumType> p_area = { px - (pw / 2), py - (ph / 2), pw, ph };
		...
		if (e.type == SDL_KEYDOWN)
		{
			if ((e.key.keysym.sym == SDLK_s || e.key.keysym.sym == SDLK_r) && e.key.repeat == 0)
			{
				...
				{
					shape_area_ = std::make_unique<Circle<int>>(mouse_pos_.x, mouse_pos_.y, circle_r_);
				}
				else
				{
					shape_area_ = std::make_unique<Rect<int>>(mouse_pos_.x - circle_r_, mouse_pos_.y - circle_r_, rect_side_, rect_side_);
				}
...
void Game::Render()
{
	...
	if (debug_areas_)
	{
		for (const auto& area : qt_->GetAreas())
		{
			...
			SDL_Rect rect = { area.top_left_.x_, area.top_left_.y_, area.width_, area.width_ };
	...
	for (const auto& qt_item : qt_->GetItems())
	{
		...
		SDL_Rect r = { qt_item.item_.top_left_.x_, qt_item.item_.top_left_.y_, qt_item.item_.width_, qt_item.item_.height_ };
	...
	if (searching_ || removing_)
	{
		...
		if (searching_)
		{
			for (const auto& qt_item_it : found_items_)
			{
				...
				SDL_Rect r = { qt_item_it->item_.top_left_.x_, qt_item_it->item_.top_left_.y_, qt_item_it->item_.width_, qt_item_it->item_.height_ };
				SDL_RenderFillRect(renderer_, &r);
		...
		{
			SDL_Rect rect = { mouse_pos_.x - circle_r_, mouse_pos_.y - circle_r_, rect_side_, rect_side_ };
